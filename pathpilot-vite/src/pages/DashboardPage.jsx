...
function parseGaps(sections) {
  const gapSection = sections.find(s =>
    s.title.toLowerCase().includes("gap") ||
    s.title.toLowerCase().includes("missing") ||
    s.title.toLowerCase().includes("weak")
  );
  if (!gapSection) return [];
  return gapSection.lines
    ...
    async function load() {
      try {
        ...
        if (histRes.ok) {
          ...
          // only redirect if no plans AND user didn't just come from onboarding
