...
function App() {
  ...
  async function handleAiPlan() {
    ...
    const planRes = await fetch(`https://pathpilot-production-de7c.up.railway.app/students/${student.id}/ai-plan`, { method: "POST" });
      ...
      <div className="max-w-xl mx-auto">
        ...
        {!token ? (
          <div className="space-y-3">
            <input className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500" name="name" placeholder="Full Name" onChange={(e) => setAuthForm({...authForm, [e.target.name]: e.target.value})} />
            <input className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500" name="email" placeholder="Email" onChange={(e) => setAuthForm({...authForm, [e.target.name]: e.target.value})} />
            <input className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500" name="password" type="password" placeholder="Password" onChange={(e) => setAuthForm({...authForm, [e.target.name]: e.target.value})} />
            <button onClick={handleRegister} className="w-full bg-green-600 hover:bg-green-500 text-white font-semibold py-3 rounded-lg transition-colors">Register</button>
            <button onClick={handleLogin} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-lg transition-colors">Login</button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <button onClick={handleAiPlan} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white font-semibold py-3 rounded-lg mt-2 transition-colors">
            ...
            {aiPlan && (
              <div className="mt-10 bg-gray-900 border border-gray-700 rounded-xl p-6">
                ...
                <div className="text-gray-300 whitespace-pre-wrap leading-relaxed text-sm">{aiPlan}</div>
              </div>
            )}
          </div>
        )}
